...
for r in get_sorted_relevant_runs(RELEVANT_KEYS):
    confidence = r.get_reset_confidence(T_STAR)
    ...
    table.append(
        [
            ...
            r.get_reset_mean(),
            r.get_reset_median(),
            r.get_reset_std_error(),
            r.get_reset_std_deviation(),
            confidence_as_text,
            base.get_reset_time_ms() / r.get_reset_time_ms(),
        ]
    )

print(tabulate(table, headers="firstrow", tablefmt="fancy_grid", floatfmt=floatfmt))

with open("table-parallel-reset", "w") as f:
    print(
